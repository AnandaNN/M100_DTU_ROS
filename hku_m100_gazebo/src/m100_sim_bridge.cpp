...
void attitudeQuaternionCallback(const geometry_msgs::QuaternionStamped::ConstPtr& attitude_quaternion_msg)
{
  ...
  target_twist.angular.x = roll*0;
  target_twist.angular.y = pitch*0;
  target_twist.angular.z = yaw*0;

  // ROS_INFO("RPY = %.1f  %.1f %.1f", roll*180/M_PI, pitch*180/M_PI, yaw*180/M_PI);
...
{
  target_twist.linear.x  = velocity_msg->vector.x*0;
  target_twist.linear.y  = velocity_msg->vector.y*0;
  target_twist.linear.z  = velocity_msg->vector.z*0;
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "m100_simulation_bridge");
  ...
  gimbal_orientation_subscriber = n.subscribe("/dji_sdk/gimbal_angle_cmd", 1000, gimbalOrientationCallback);
  ...
  target_gimbal_pose.orientation.y = 0;
  target_gimbal_pose.orientation.z = 0;
