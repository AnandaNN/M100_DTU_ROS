...
int main(int argc, char** argv)
{
  ...
  rodSub = nh.subscribe<std_msgs::Bool>("/dji_sdk/rod", 1, rodCallback);
  
  // Basic services
  if( sim )
    ROS_INFO("GOT CONTROL IN SIMULATION!");
  else
  {
  ...
  if( sim ) ROS_INFO("CONTROL RELEASED IN SIMULATION!");
  else if( release_control() ) ROS_INFO("CONTROL RELEASED!");
