...
void ContactController::controlTimerCallback( const ros::TimerEvent& )
{
  ...
  if( _rodValue )
  {
    ...
    _controlValue.axes[0] = _kpYaw * (_lastYaw - _attitude.z) - _kdYaw * _angularVelocityWorldFrame.getZ();
    _controlValue.axes[2] = _kpPitch * (_targetPitch - rad2deg * _attitude.y);

    _contactBuffer = 0;
  }
  else
  {
    _lastYaw = _attitude.z;
    _contactBuffer++;
