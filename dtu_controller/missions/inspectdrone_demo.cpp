/** @file inspectdrone_demo.cpp
 *  @version 1.0
 *  @date November, 2020
 *
 *  @brief
 *  Mission for doing autonomous wall touch with visual servoing and user inputs with autonomous takeoff and landing
 *
...
int main(int argc, char** argv)
{
  ...
  ROS_INFO("Ready to take off. Waiting for command");
  char input;
  std::cin >> input;
  ...
  arm_motors(nh, true);
  ros::Duration(1).sleep();
  controllerInterface.set_reference(0,0,1.5,0);
  ...
  while(ros::ok()) 
  {
    ...
    if( trackerEnabled.data == (uint8_t) 1 )
    {
      ...
      controllerInterface.switchPositioning( nh, VISUAL_ODOMETRY );
  ...
  ROS_INFO("Centering around the target! (3s) %f", xTarget);
  ...
  controllerInterface.switchPositioning( nh, GPS );
  ros::Duration(3).sleep();
